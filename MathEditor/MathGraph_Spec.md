...
### 5.3 立體積木 (Isometric Solid Blocks)
- 使用等角投影繪製的堆疊積木。
- `data-heights`: 必須是 JSON 格式的二維陣列 `[[h,h],[h,h]]`。陣列索引 `row=0` 代表**最前方**，`col=0` 代表**最左方**。
- `data-block-size`: 設定單一積木的像素大小 (預設 35)。
- `data-show-arrows`: 布林值 (`true`/`false`)，控制是否顯示「上方、前方、右方」的指示箭頭與文字。

```xml
<g class="shape group solid-blocks" data-tool="blocks" data-heights="[[2,1,0],[1,1,0],[1,0,0]]" data-block-size="35" data-show-arrows="true" transform="translate(400, 300)">
    <!-- 內部的 polygon (積木面) 與 arrow (箭頭) 由系統根據 heights 自動生成，AI 僅需產出此容器 -->
</g>
```

### 5.4 三視圖 (Orthographic Views)
- 對應立體積木的平面視圖（上視圖、前視圖、右視圖）。
- `data-view-type`: 必須為 `"top"` (上)、`"front"` (前) 或 `"right"` (右)。
- `data-owner-blocks`: 必須填入對應立體積木的 `id`，以建立連動關係。

```xml
<!-- 上視圖範例 -->
<g class="shape group" data-tool="group" data-view-type="top" data-owner-blocks="blocks-timestamp-id" transform="translate(300, 500)">
    <!-- 內部的 rect (格子) 與 text (層數數字) 由系統自動生成 -->
</g>

<!-- 前視圖範例 -->
<g class="shape group" data-tool="group" data-view-type="front" data-owner-blocks="blocks-timestamp-id" transform="translate(450, 500)">
</g>

<!-- 右視圖範例 -->
<g class="shape group" data-tool="group" data-view-type="right" data-owner-blocks="blocks-timestamp-id" transform="translate(600, 500)">
</g>
```
