# 數學公式繪圖編輯器 (MathGraph) - SVG 元件生成規格書 v1.2 (完整版)
...
- 必須包裝在 `<foreignObject>` 與 `<div>` 結構中，且 div 內容必須用單反引號 ` 包住。
...
若要畫出角度弧線與文字，父圖形必須加上 `data-angle-label-ids="弧線ID,文字ID"`，否則系統將無法自動刪除它們，會造成重疊！
...
- 必須有一個主體 `<ellipse>` 帶 ID。群組必須有 `data-owner-circle-id="主體圓ID"`。
- 必須包含對應數量的 `<circle class="vertex-data">` 作為隱藏的控制把手。
- 若有角度標示，群組必須加上 `data-angle-label-ids`！
...
### 5.2 立體圖形 (Solids - Cube / Cylinder / Cone)
